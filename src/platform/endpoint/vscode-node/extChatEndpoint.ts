...
export class ExtensionContributedChatEndpoint implements IChatEndpoint {
	...
	async makeChatRequest2({
		...
		try {
			...
			let usageFromProvider: APIUsage | undefined;
			...
			for await (const chunk of response.stream) {
				if (chunk instanceof vscode.LanguageModelTextPart) {
					...
					if (chunk.mimeType === CustomDataPartMimeTypes.StatefulMarker) {
						...
						usageFromProvider = JSON.parse(new TextDecoder().decode(chunk.data)) as APIUsage;
					}
			...
			if (text || numToolsCalled > 0) {
				return {
					...
					usage: usageFromProvider ?? { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0, prompt_tokens_details: { cached_tokens: 0 } },
					value: text,
